...
pdf(paste('TNBC_slopes_',filter,'_', f_test[1], '.pdf', sep = ''))
TNBC_df$Label <- factor(TNBC_df$Label, unique(as.character(TNBC_df$Label)))
ggplot(data=TNBC_df, aes(x=Label, y=Slope)) + geom_boxplot(fill = c('red', 'darkorange', 'green','blue','purple')) + ggtitle(paste("TNBC data -1 slopes, ",xxxx, "fold filtering\nfor f values: ",paste(f_test,collapse=", "), sep=''))
#ggplot(data=TNBC_df, aes(x=Label, y=Slope)) + geom_boxplot(fill = c('red', 'darkorange', 'purple','green','blue')) + ggtitle(paste("TNBC data -1 slopes\nfor f values: ",paste(f_test,collapse=", "), sep=''))  + geom_line(aes( x, y, linetype = cutoff ), cutoff)
dev.off()

pdf(paste('ER_slopes_',filter,'_', f_test[1], '.pdf', sep = ''))
ER_df$Label <- factor(ER_df$Label, unique(as.character(ER_df$Label)))
ggplot(data=ER_df, aes(x=Label, y=Slope)) + geom_boxplot(fill = c('red', 'darkorange', 'green','blue','purple')) + ggtitle(paste("ER data -1 slopes, ",xxxx, "fold filtering\nfor f values: ",paste(f_test,collapse=", "), sep=''))
