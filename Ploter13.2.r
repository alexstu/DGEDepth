...
ER_df_regr_all <- matrix(ncol=3, nrow=length(methods)*length(ER_samples)*length(N_boots)*length(depths))
TNBC_df_regr_all <- matrix(ncol=3, nrow=length(methods)*length(ER_samples)*length(N_boots)*length(depths))
colnames(ER_df_regr_all) = c('Label', 'Variable', 'Slope')
colnames(TNBC_df_regr_all) = c('Label', 'Variable', 'Slope')
TNBC_df_regr_all <- as.data.frame(TNBC_df_regr_all)
ER_df_regr_all <- as.data.frame(ER_df_regr_all)
	...
	for (k in 1:11){
		...
		for (i in 1:length(depths)){
			...
			for (j in 1:length(N_boots)){
				...
				FDR <- (FP/(TP+FP))
				if (length(ER_results[[m]][[k]][[i]][[j]]) == 0){
					FDR=1
				}
				depth_ER_list[[j]] <- FDR
				if (is.na(FDR))
					print(paste(m,k,i,j, sep='	'))
				...
				FDR <- (FP/(TP+FP))
				if (length(TNBC_results[[m]][[k]][[i]][[j]]) == 0){
					FDR=1
				}
...
########################
ER_df_all <- matrix(ncol=3, nrow=length(methods)*length(ER_samples)*length(N_boots)*length(depths))
TNBC_df_all <- matrix(ncol=3, nrow=length(methods)*length(ER_samples)*length(N_boots)*length(depths))
colnames(ER_df_all) = c('Label', 'Variable', 'Slope')
colnames(TNBC_df_all) = c('Label', 'Variable', 'Slope')
TNBC_df_all <- as.data.frame(TNBC_df_all)
ER_df_all <- as.data.frame(ER_df_all)
	...
	for (k in 1:11){
		for (i in 1:length(depths)){
			for (j in 1:length(N_boots)){
				...
				qqq <- 11*(m-1)*length(N_boots)*length(depths)+length(N_boots)*length(depths)*(k-1)+length(N_boots)*(i-1)+j
				ER_df_all[qqq,1] <- methods[m]
				ER_df_all[qqq,2] <- depths[i]
				ER_df_all[qqq,3] <- ER_FDR_list[[m]][[k]][[i]][[j]]
...
11*(m-1)*length(N_boots)*length(depths)+length(N_boots)*length(depths)*(k-1)+length(N_boots)*(i-1)+j
...
TNBC_df$Label <- factor(TNBC_df$Label, unique(as.character(TNBC_df$Label)))
#ggplot(data=TNBC_df, aes(x=Label, y=Slope)) + geom_boxplot(fill = c('red', 'darkorange', 'green','blue','purple')) + ggtitle(paste("TNBC data -1 slopes\nfor f values: ",paste(f_test,collapse=", "), sep=''))
ggplot(data=TNBC_df, aes(x=Label, y=Slope)) + geom_boxplot(fill = c('red', 'darkorange', 'purple','green','blue')) + ggtitle(paste("TNBC data -1 slopes\nfor f values: ",paste(f_test,collapse=", "), sep='')) + xlab("Software") + ylim(-0.01, 0.01)
...
ER_df$Label <- factor(ER_df$Label, unique(as.character(ER_df$Label)))
...
write.table(TNBC_matrix, 'TNBCslopestable.txt', quote=F)
write.table(ER_matrix, 'ERslopestable.txt', quote=F)



d <- posthoc.friedman.conover.test(TNBC_matrix)
write.table(d$p.value, 'TNBCconover.txt', quote = F, sep = '\t')

d <- posthoc.friedman.nemenyi.test(TNBC_matrix)
write.table(d$p.value, 'TNBCnemenyi.txt', quote = F, sep = '\t')

d <- posthoc.friedman.conover.test(ER_matrix)
write.table(d$p.value, 'ERconover.txt', quote = F, sep = '\t')

d <- posthoc.friedman.nemenyi.test(ER_matrix)
write.table(d$p.value, 'ERnemenyi.txt', quote = F, sep = '\t')


friedman.test(Slope ~ Label | Variable, data=TNBC_df)
friedman.test(Slope ~ Label | Variable, data=ER_df)
