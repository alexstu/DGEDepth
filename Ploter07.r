...
load(paste('sub_de_ER_deseq_0.05_',filter, sep = ''))
load(paste('sub_de_ER_ebseq_0.05_',filter, sep = ''))
load(paste('sub_de_ER_edger_0.05_',filter, sep = ''))
load(paste('sub_de_ER_noiseq_0.05_',filter, sep = ''))
load(paste('sub_de_ER_voom_0.05_',filter, sep = ''))
load(paste('sub_de_TNBC_deseq_0.05_',filter, sep = ''))
load(paste('sub_de_TNBC_ebseq_0.05_',filter, sep = ''))
load(paste('sub_de_TNBC_edger_0.05_',filter, sep = ''))
load(paste('sub_de_TNBC_noiseq_0.05_',filter, sep = ''))
load(paste('sub_de_TNBC_voom_0.05_',filter, sep = ''))
...
for (i in 1:length(depths)){
	for (j in 1:length(N_boots)){
		...
		ER_df[5*((i-1)*length(N_boots)+(j-1))+1,3] <- (TP/(TP+FP))
		...
		ER_df[5*((i-1)*length(N_boots)+(j-1))+2,3] <- (TP/(TP+FP))
		...
		ER_df[5*((i-1)*length(N_boots)+(j-1))+3,3] <- (TP/(TP+FP))
		...
		ER_df[5*((i-1)*length(N_boots)+(j-1))+4,3] <- (TP/(TP+FP))
		...
		ER_df[5*((i-1)*length(N_boots)+(j-1))+5,3] <- (TP/(TP+FP))
		...
		TNBC_df[5*((i-1)*length(N_boots)+(j-1))+1,3] <- (TP/(TP+FP))
		...
		TNBC_df[5*((i-1)*length(N_boots)+(j-1))+2,3] <- (TP/(TP+FP))
		...
		TNBC_df[5*((i-1)*length(N_boots)+(j-1))+3,3] <- (TP/(TP+FP))
		...
		TNBC_df[5*((i-1)*length(N_boots)+(j-1))+4,3] <- (TP/(TP+FP))
		...
		TNBC_df[5*((i-1)*length(N_boots)+(j-1))+5,3] <- (TP/(TP+FP))

		}
}

plotsamExplorer(ER_df, save = TRUE, p.depth = 0.9, filename = paste('sub_PR_ER_fold_',filter, sep = ''),anova = FALSE,y.lab="Precision", leg.name = 'Methods', title = paste('Precision for (5+5) samples of ER+ data, ',xxxx, 'fold filtering',sep=''))
system('sleep 5')
plotsamExplorer(TNBC_df, save = TRUE, p.depth = 0.9, filename = paste('sub_PR_TNBC_fold_',filter, sep = ''),anova = FALSE,y.lab="Precision", leg.name = 'Methods', title = paste('Precision for (5+5) samples of TNBC data, ',xxxx, 'fold filtering',sep=''))



length(Reduce(intersect, list(de_ER_deseq[[17]][[1]], de_ER_ebseq[[17]][[1]], de_ER_edger[[17]][[1]], de_ER_noiseq[[17]][[1]], de_ER_voom[[17]][[1]])))
length(Reduce(intersect, list(de_TNBC_deseq[[17]][[1]], de_TNBC_ebseq[[17]][[1]], de_TNBC_edger[[17]][[1]], de_TNBC_noiseq[[17]][[1]], de_TNBC_voom[[17]][[1]])))
