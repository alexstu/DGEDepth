	...
	for (s in 1:length(samples)){
		...
		for (i in 1:length(depths)){
			...
			boot_list <- foreach(j = 1:length(N_boots)) %dopar% {
				...
				if (s == 1){
					x <- as.matrix(x)
					pattern_files_list <- sub_names
					if (type == 'ER+'){
						type_factor = (c(rep(1,5), rep(0,5)))
					}
					if (type == 'TNBC'){
						type_factor = (c(rep(1,5), rep(0,5)))
					}
...
save('de_ER_voom', file = paste('sub_JK_de_ER_voom_',as.character(pval), '_', as.character(fold), sep = ''))
save('de_TNBC_voom', file = paste('sub_JK_de_TNBC_voom_',as.character(pval), '_', as.character(fold), sep = ''))
