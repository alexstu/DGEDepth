		...
		for (i in 1:length(depths)){
			...
			boot_list <- foreach(j = 1:length(N_boots)) %dopar% {
				...
				if (s == 1){
					x <- x
					x <- as.matrix(x)
					pattern_files_list <- as.character(colnames(x))
					if (type == 'ER+'){
						type_factor = (c(rep(1,42), rep(0,30)))
					}
					if (type == 'TNBC'){
						type_factor = (c(rep(1,42), rep(0,21)))
					}
				}
				if ((s > 1) & (s <= 6)){
					x <- x[, colnames(x) != samples[s]]
					x <- as.matrix(x)
					pattern_files_list <- as.character(colnames(x)[colnames(x) != samples[s]])
					if (type == 'ER+'){
						type_factor = (c(rep(1,41), rep(0,30)))
					}
					if (type == 'TNBC'){
						type_factor = (c(rep(1,41), rep(0,21)))
					}
				}
				if (s > 6){
					x <- x[, colnames(x) != samples[s]]
					...
					pattern_files_list <- as.character(colnames(x)[colnames(x) != samples[s]])
					if (type == 'ER+'){
						type_factor = (c(rep(1,42), rep(0,29)))
					}
					if (type == 'TNBC'){
						type_factor = (c(rep(1,42), rep(0,20)))
					}
				}
			...
			v <- voom(x,design,plot=F,normalize="quantile")
			fit <- lmFit(v,design)
			fit <- eBayes(fit)
			
			topTable(fit, n = dim(fit)[1],coef = 'V2', sort.by = 'none', adjust="BH")
			#boot_list[[j]] <- de
...
for (s in 1:length(samples)){
	...
	de_ER_voom[[s]] <- ER_sample_list
	de_TNBC_voom[[s]] <- TNBC_sample_list

}

save('de_ER_voom', file = paste('JK_de_ER_voom_',as.character(pval), '_', as.character(fold), sep = ''))
save('de_TNBC_voom', file = paste('JK_de_TNBC_voom_',as.character(pval), '_', as.character(fold), sep = ''))
