		...
		for (i in 1:length(depths)){
			...
			boot_list <- foreach(j = 1:length(N_boots)) %dopar% {
				...
				if (s == 1){
					x <- x
					if (type == 'ER+'){
						colDat = data.frame(condition = factor(c(rep(1,5), rep(2,5))))
					}
					if (type == 'TNBC'){
						colDat = data.frame(condition = factor(c(rep(1,5), rep(2,5))))
					}
				...
				if ((s > 1) & (s <= 6)){
					...
					if (type == 'ER+'){
						colDat = data.frame(condition = factor(c(rep(1,4), rep(2,5))))
					}
					if (type == 'TNBC'){
						colDat = data.frame(condition = factor(c(rep(1,4), rep(2,5))))
					}
				...
				if (s > 6){
					x <- x[, colnames(x) != samples[s]]
					if (type == 'ER+'){
						colDat = data.frame(condition = factor(c(rep(1,5), rep(2,4))))
					}
					if (type == 'TNBC'){
						colDat = data.frame(condition = factor(c(rep(1,5), rep(2,4))))
					}
...
save.image('sub_JKDEseq2.RData')

#############################

load('sub_JKDEseq2.RData')
