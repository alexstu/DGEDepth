...
load('Scripts/-1/JK_de_ER_deseq_0.05_2')
load('Scripts/-1/JK_de_TNBC_deseq_0.05_2')
...
for (i in 1:length(depths)){
	...
	go_ER_deseq[[i]] <- ER_boot_list
	go_TNBC_deseq[[i]] <- TNBC_boot_list
}

save('go_ER_deseq', file = paste('go_ER_deseq_',as.character(pval), '_', as.character(fold), sep = ''))
save('go_TNBC_deseq', file = paste('go_TNBC_deseq_',as.character(pval), '_', as.character(fold), sep = ''))
