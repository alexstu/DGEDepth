...
load('Scripts/-1/JK_de_ER_noiseq_0.05_2')
load('Scripts/-1/JK_de_TNBC_noiseq_0.05_2')
...
for (i in 1:length(depths)){
	...
	for (j in 1:length(N_boots)){
		...
		x <- de_ER_noiseq[[1]][[i]][[j]]
		y <- de_TNBC_noiseq[[1]][[i]][[j]]
...
save('go_ER_noiseq', file = paste('go_ER_noiseq_',as.character(pval), '_', as.character(fold), sep = ''))
save('go_TNBC_noiseq', file = paste('go_TNBC_noiseq_',as.character(pval), '_', as.character(fold), sep = ''))
