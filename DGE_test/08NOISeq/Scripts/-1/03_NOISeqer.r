		...
		for (i in 1:length(depths)){
			...
			boot_list <- foreach(j = 1:length(N_boots)) %dopar% {
				...
				if (s == 1){
					x <- x
					if (type == 'ER+'){
						y = data.frame(Group = factor(c(rep(1,42), rep(0,30))))
					}
					if (type == 'TNBC'){
						y = data.frame(Group = factor(c(rep(1,42), rep(0,21))))
					}
				}
				if ((s > 1) & (s <= 6)){
					x <- x[, colnames(x) != samples[s]]
					if (type == 'ER+'){
						y = data.frame(Group = factor(c(rep(1,41), rep(0,30))))
					}
					if (type == 'TNBC'){
						y = data.frame(Group = factor(c(rep(1,41), rep(0,21))))
					}
				}
				if (s > 6){
					x <- x[, colnames(x) != samples[s]]
					if (type == 'ER+'){
						y = data.frame(Group = factor(c(rep(1,42), rep(0,29))))
					}
					if (type == 'TNBC'){
						y = data.frame(Group = factor(c(rep(1,42), rep(0,20))))
					}
				}
...
save('de_ER_noiseq', file = 'JK_de_ER_noiseq_0.05_none')

de_TNBC_noiseq <- de_TNBC_noiseq_no_fold
save('de_TNBC_noiseq', file = 'JK_de_TNBC_noiseq_0.05_none')

de_ER_noiseq <- de_ER_noiseq_fold
save('de_ER_noiseq', file = 'JK_de_ER_noiseq_0.05_2')

de_TNBC_noiseq <- de_TNBC_noiseq_fold
save('de_TNBC_noiseq', file = 'JK_de_TNBC_noiseq_0.05_2')
