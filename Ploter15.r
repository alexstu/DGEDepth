'-1 FDR~d slopes for 5+5 samples'
library(ggplot2)
library(samExploreR)
library(PMCMR)
...
cutoff <- data.frame( x = c(-Inf, Inf), y = -0.050, cutoff = factor(-0.050) )
pdf(paste('sub_TNBC_slopes_',filter,'_', f_test[1], '.pdf', sep = ''))
...
#ggplot(data=TNBC_df, aes(x=Label, y=Slope)) + geom_boxplot(fill = c('red', 'darkorange', 'purple','green','blue')) + ggtitle(paste("TNBC data -1 slopes for 5+5 samples, ",xxxx, "fold filtering\nfor f values: ",paste(f_test,collapse=", "), sep=''))  + geom_line(aes( x, y, linetype = cutoff ), cutoff) + theme(plot.title = element_text(hjust=0.5)) + xlab("Software") + ylim(-0.1, 0.00)
ggplot(data=TNBC_df, aes(x=Label, y=Slope)) + geom_boxplot(fill = c('red', 'darkorange', 'purple','green','blue')) + ggtitle(paste("TNBC data -1 slopes for 5+5 samples, ",xxxx, "fold filtering\nfor f values: ",paste(f_test,collapse=", "), sep='')) + xlab("Software") + ylim(-0.1, 0.00)
dev.off()

pdf(paste('sub_ER_slopes_',filter,'_', f_test[1], '.pdf', sep = ''))
...
#ggplot(data=ER_df, aes(x=Label, y=Slope)) + geom_boxplot(fill = c('red', 'darkorange', 'purple','green','blue')) + ggtitle(paste("ER data -1 slopes for 5+5 samples, ",xxxx, "fold filtering\nfor f values: ",paste(f_test,collapse=", "), sep=''))  + geom_line(aes( x, y, linetype = cutoff ), cutoff) + theme(plot.title = element_text(hjust=0.5)) + xlab("Software") + ylim(-0.1, 0.00)
ggplot(data=ER_df, aes(x=Label, y=Slope)) + geom_boxplot(fill = c('red', 'darkorange', 'purple','green','blue')) + ggtitle(paste("ER data -1 slopes for 5+5 samples, ",xxxx, "fold filtering\nfor f values: ",paste(f_test,collapse=", "), sep='')) + xlab("Software") + ylim(-0.1, 0.00)
dev.off()


write.table(TNBC_matrix, 'subTNBCslopestable.txt', quote=F)
write.table(ER_matrix, 'subERslopestable.txt', quote=F)




d <- posthoc.friedman.conover.test(TNBC_matrix)
write.table(d$p.value, 'subTNBCconover.txt', quote = F, sep = '\t')

d <- posthoc.friedman.nemenyi.test(TNBC_matrix)
write.table(d$p.value, 'subTNBCnemenyi.txt', quote = F, sep = '\t')

d <- posthoc.friedman.conover.test(ER_matrix)
write.table(d$p.value, 'subERconover.txt', quote = F, sep = '\t')

d <- posthoc.friedman.nemenyi.test(ER_matrix)
write.table(d$p.value, 'subERnemenyi.txt', quote = F, sep = '\t')
